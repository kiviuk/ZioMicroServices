...
object RequestTest extends ZIOSpecDefault {
  ...
  def spec: Spec[Any, Nothing] = suite("Request")(
    test("Should create request") {
      val r = InsideElevatorRequest(1, stats = RequestStatistics())
      assertTrue(r.creationTime.isBefore(Instant.now().plusMillis(1000)))
    },
    test("Should order requests by descending floor number") {

      // arrange
      val topFloor = InsideElevatorRequest(100, stats = RequestStatistics())
      val bottomFloor = InsideElevatorRequest(1, stats = RequestStatistics())
    ...
    test("Should order requests by descending floor number2") {
      ...
      // arrange
      val bottomFloor = InsideElevatorRequest(-100, stats = RequestStatistics())
      val topFloor = InsideElevatorRequest(1, stats = RequestStatistics())
