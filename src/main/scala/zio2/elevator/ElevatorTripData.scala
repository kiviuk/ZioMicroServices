...
case class ElevatorTripData(
    servedByElevator: Option[String] = None,
  ...
  def getTripTimeInMillis: Option[Double] =
    pickedUpAt.flatMap(pt => getMillisBetween(pt, droppedOffAt))
  ...
  private def getDroppedOffAtFloor: Int =
    droppedOffAtFloor.getOrElse(Int.MaxValue)
  ...
  override def toString: String = {
    ...
    s"$getServedByElevator;" +
      s" PickedUpOnFloor: $getPickedUpOnFloor;" +
      s" DroppedOffAtFloor: $getDroppedOffAtFloor;" +
      s" floorDistance: $floorDistance;" +
      s" totalTime: $totalTimeStr;" +
      s" tripTime: $tripTime;" +
      s" waitingTime: $waitingTimeStr"
  }
}
